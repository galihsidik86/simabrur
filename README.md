...
Implementasi full-stack dari mockup desain di repo ini (file `*.dc.html`). Rencana lengkap dan pembagian fase: lihat **PLAN.md**. Panduan pemakaian per peran dan alur transaksi: **[docs/PANDUAN-PENGGUNA.md](docs/PANDUAN-PENGGUNA.md)** (versi cetak/PDF: [docs/panduan-pengguna.html](docs/panduan-pengguna.html) — buka di browser → Cetak).
